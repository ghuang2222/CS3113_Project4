...
enum AIType { WALKER, GUARD, STALKER, SHOOTER, BULLET};
enum AIState { WALKING, IDLE, RELOADING, ATTACKING };
...
private:
    ...
    AnimationDirection m_direction = RIGHT;
    ...
    glm::vec3 m_init_pos;
    float m_shoot_interval = 2.0f;
    float m_reload_time = 0.0f;
    // ————— TRANSFORMATIONS ————— //
    glm::vec3 m_movement;
    glm::vec3 m_position;
    glm::vec3 m_scale;
    ...
    bool m_collided_low_right_corner = false;
    bool m_collided_low_left_corner = false;
    Entity* m_collided_with;
...
public:
    ...
    //void update(float delta_time, Entity* player, std::vector<Entity*> collidable_entities, int collidable_entity_count, Map* map);
    ...
    void ai_bullet(Entity* player);

    void normalise_movement() { m_movement = glm::normalize(m_movement); }

    void face_left() { m_animation_indices = m_walking[LEFT]; m_direction = LEFT; }
    void face_right() { m_animation_indices = m_walking[RIGHT]; m_direction = RIGHT; }
    ...
    AnimationDirection const get_direction() const { return m_direction; }
    // ————— SETTERS ————— //
    ...
    void  set_collided_bottom(bool new_collided) { m_collided_bottom = new_collided; }
    void set_collided_top(bool new_collided) { m_collided_top = new_collided; }
    void  set_collided_right(bool new_collided) { m_collided_right = new_collided; }
    void set_collided_left(bool new_collided) { m_collided_left = new_collided; }

    void set_init_pos(glm::vec3 init_pos) { m_init_pos = init_pos; }

    void set_bullet(Entity* new_bullet) {
        m_bullet = new_bullet;
        
        m_bullet->m_ai_type = BULLET;
    }
